        ...
        async with httpx.AsyncClient(timeout=timeout, headers=headers) as client:
            ...
            for item in data.get("vulnerabilities", []):
                ...
                findings.append({
                    ...
                    "port":            str(port),
                    "url":             f"nvd://{target}:{port}/{cve_id}" if port and port != "N/A" else f"nvd://{target}/{cve_id}",
                    "payload_url":     f"nvd://{target}:{port}/{cve_id}" if port and port != "N/A" else f"nvd://{target}/{cve_id}",
                    "module":          "nvd",
