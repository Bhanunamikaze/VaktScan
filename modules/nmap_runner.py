                                ...
                                for vuln_table in cpe_table.findall('table'):
                                    ...
                                    if vuln_id.startswith('CVE-') and cvss_score >= 7.0:
                                        ...
                                        findings.append({
                                            ...
                                            'resolved_ip': ip,
                                            'port': str(port_id),
                                            'url': f"nmap://{hostname}:{port_id}/{vuln_id}",
                                            'payload_url': f"nmap://{hostname}:{port_id}/{vuln_id}",
                                            'module': 'nmap',
                                            'service_version': version or 'N/A',
                                            'details': f"Nmap vulners script detected {vuln_id} with CVSS {cvss_score:.1f}. Reference: https://nvd.nist.gov/vuln/detail/{vuln_id}",
                                            'http_status': 'N/A',
                                            'page_title': 'N/A',
                                            'content_length': 'N/A',
                                    ...
                                    for cve in cves:
                                        ...
                                        if cvss_score == 0.0 or cvss_score >= 7.0:
                                            ...
                                            findings.append({
                                                ...
                                                'resolved_ip': ip,
                                                'port': str(port_id),
                                                'url': f"nmap://{hostname}:{port_id}/{cve_upper}",
                                                'payload_url': f"nmap://{hostname}:{port_id}/{cve_upper}",
                                                'module': 'nmap',
                                                'service_version': version or 'N/A',
                                                'details': f"Nmap script {script_id} detected {cve_upper} on port {port_id}. Reference: https://nvd.nist.gov/vuln/detail/{cve_upper} Output: {output[:300]}",
                                                'http_status': 'N/A',
                                                'page_title': 'N/A',
                                                'content_length': 'N/A',
                                                'timestamp': datetime.utcnow().isoformat() + 'Z',
