...
Input: <domain | IP | CIDR | file>
  ...
  │  ├─ DNS Recon ──────────────────── parallel ──────┐ │
  │  ├─ Cloud Enum (S3/Azure/GCP) ── parallel ──────┐ │ │
  │  └─ Google Dork ─────────────── parallel ──────┐ │ │ │
  │            │ (wait for enum)                    │ │ │ │
  └────────────┼────────────────────────────────────┘─┘─┘─┘
               ▼
         Port Scanner
    ...
    │          │                        │
    ▼          ▼                        ▼
  Nuclei   Web Checks           Service Modules
  (CVEs)  (git/env/admin)   elastic│kibana│grafana
                            prometheus│cpanel│jenkins
                            service_recon│aem│...
    └──────────┼────────────────────────┘
               ▼
          Enrichment
     CISA KEV → EPSS → NVD → Passive Intel
               │
               ▼
         Reporter
  CSV + JSON + SARIF → reports/<target>_<ts>/
```
