...
async def validate_elasticsearch(ip, port, timeout=5):
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout, verify=False) as client:
            ...
            response = await client.get(f"http://{ip}:{port}/_cluster/health")
            if response.status_code == 200:
                content = response.text.lower()
...
async def validate_kibana(ip, port, timeout=5):
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout, verify=False) as client:
            ...
            response = await client.get(f"http://{ip}:{port}/app/kibana")
...
async def validate_grafana(ip, port, timeout=5):
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout, verify=False) as client:
            ...
            # Try login page
            response = await client.get(f"http://{ip}:{port}/login")
            if response.status_code == 200:
                content = response.text.lower()
                if 'grafana' in content:
                    return True
            ...
            response = await client.get(f"http://{ip}:{port}/api/health")
            if response.status_code == 200:
                content = response.text.lower()
...
async def validate_prometheus(ip, port, timeout=5):
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout, verify=False) as client:
            ...
            response = await client.get(f"http://{ip}:{port}/metrics")
            if response.status_code == 200:
            ...
            response = await client.get(f"http://{ip}:{port}/api/v1/query?query=up")
            if response.status_code == 200:
                content = response.text.lower()
            ...
            # Try graph endpoint
            response = await client.get(f"http://{ip}:{port}/graph")
            if response.status_code == 200:
                content = response.text.lower()
                if 'prometheus' in content:
                    return True
